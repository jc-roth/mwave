...
def cloud_to_scrbi_ellipse_xy(x0, y0, z0, vx, vy, vz, T, Tp, n, N, phi_c, phi_d, bragglookup, omegalookup, lplookup, deltalookup, incl_junk=False):
    ...
    :param lplookup: Function that returns the local laser phase at the specified location. Must accept arguments :code:`x, y, z`, where all arguments are vectors of the same length. Note that this should return the kvector, and not the effective kvector as the :py:meth:`mwave.simulation_utils.cloud_to_scrbi_ellipse_xy` function will apply a factor of two the the phase returned by :code:`lplookup`.
